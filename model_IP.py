...
def MSpaRB(X, filters, stride, validkernel=(1,1,1)):
    ...
    X = BN(X)
    X_shortcut = X
    branch_1 = Conv3D(filters= F2, kernel_size= (1, 1, 1), strides= (1, 1, 1), padding= 'same', kernel_initializer= kernel_init)(X)
    branch_2 = Conv3D(filters=F2, kernel_size=(3, 3, 1), strides=(1, 1, 1), padding='same',
                      kernel_initializer=kernel_init)(X)
    # branch_3 = Conv3D(filters= F2, kernel_size= (5, 5, 1), strides= (1, 1, 1), padding= 'same', kernel_initializer= kernel_init)(X)
    # branch_4 = Conv3D(filters= F2, kernel_size= (7, 7, 1), strides= (1, 1, 1), padding= 'same', kernel_initializer= kernel_init)(X)

    X = concatenate([branch_1, branch_2], axis=4)
    X = Conv3D(filters= F3, kernel_size=(1, 1, 1), strides=(1, 1, 1), kernel_initializer= kernel_init)(branch_1)
    X = BN(X)
    X = Add()([X, X_shortcut])
    X = Activation('relu')(X)
    X = Conv3D(filters= F3, kernel_size= validkernel, strides= (1 ,1, 1), padding= 'valid', kernel_initializer= kernel_init)(X)
...
def SCAM(X):
    ...
    conv_band4x4 = Conv3D(1, kernel_size=(1, 1, 1), strides=(1, 1, 1), padding='same', kernel_initializer=kernel_init)(
        X)
...
def unet(pretrained_weights=None, input_size=(patch_size, patch_size, 200, 1)):
    ...
    conv0b = MSpeRB(conv00p,filters=[32,32,64],stride=strides112)
    ...
    conv2a = MSpaRB(conv22u, filters=[64, 64, 128], stride=strides112,validkernel=(1, 1, 2))
    ...
    #    conv_1 = Conv3D(filters = 512, kernel_size = (1,1,2), strides = (1,1,1), kernel_initializer = kernel_init,padding='valid')(conv6)
